...
מדיניות עסקית שאינה ידועה - עלות משלוח, הנחות, החזרות, אחריות או תמחור הזמנה
אישית - ומגדיר בסבב הראשון `ANY` עם `offer_whatsapp` ככלי המותר היחיד. כך
הסירוב והכפתור מובטחים בלי להפוך כל ברכה לחיפוש קטלוג.
...
מורכבים בתבניות קבועות בקוד מרשומת הכלי; טקסט המודל אינו מקור לנתונים אלה.
...
* זמינות נקראת תמיד משדה `availability` של המוצר, וזמני המסירה נקראים מ־
  `lib/fulfillment.ts`; כתובת הסטודיו ושעותיו נקראות מ־`lib/constants.ts` דרך
  אותה קריאת חיפוש. העוזר עונה על הנושאים האלה רק מתוצאת כלי באותו תור. דמי
  משלוח, מבצעים, החזרות, אחריות ותמחור הזמנה אישית אינם ידועים ומקבלים תשובה
  כנה והפניה לוואטסאפ. ידע כללי על תכשיטים יכול להיענות ללא כלי כל עוד אינו
  מיוחס למוצר או למדיניות של נוגה.
