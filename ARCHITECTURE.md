...
הסבב הראשון מול Gemini מוגדר `ANY`, ולכן הוא חייב להחזיר קריאת כלי ולא יכול
לענות בפרוזה בלבד; הסבבים הבאים מוגדרים `AUTO` כדי לאפשר תשובת סיום. הוראת
המערכת מחייבת שכל בקשת קנייה, מתנה, העדפה או גלישה פתוחה תתחיל ב־
`search_products`. אם אין מסנן קטלוג שימושי, קריאה ללא ארגומנטים מחזירה מבחר
רחב ורק לאחר מכן העוזר מצמצם או שואל שאלה אחת. קריאת מדיניות מסירה ללא מסנני
מוצר נשארת קריאת מדיניות בלבד ואינה מציגה כרטיסים לא קשורים.
...
הלשונית ובחירת הכלי/השדות לאחר החיוב לקרוא כלי: `ANY` מחייב כלי, אבל רק הוראת
המערכת יכולה לחייב שהכלי שנבחר לבקשת גלישה יהיה דווקא `search_products`.
טעות שם יכולה לתת תשובה לא רלוונטית, אבל אינה יכולה ליצור כרטיס או נתון
קטלוג חדש.

הלוגים בצד השרת מינימליים: לכל קריאה נרשמים שם הכלי, ארגומנטים בטוחים ומספר
התוצאות; לאחר מכן נרשם אם סורק ההארקה החליף את הטקסט. ערך `query` עצמו מוחלף
בדגל אורך/נוכחות, ולא נרשמים תוכן משתמשת, מפתח API או מזהה סשן.
