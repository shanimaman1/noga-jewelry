...
| מה | איפה | הערה |
| --- | --- | --- |
| פריטים, מחירים, מתכות, אבנים, משקל זהב וזמינות | `src/data/products.ts` | 16 פריטים, ₪890–₪8,900 |
...
`availableIn18K` קובע אם להציג אפשרות הזמנה בזהב 18 קראט. אין טקסט ברירת
...
על משקל כולל של כמה יהלומים. כלי הקטלוג של העוזר מחזיר גם את
`availableIn18K`; בשאלה על 18 קראט הוא מציג תשובה קבועה שהקוד מרכיב מהשדה,
ולא ניסוח עובדתי של המודל.
...
מדיניות עסקית שאינה ידועה - הנחות, החזרות, אחריות או תמחור הזמנה אישית -
...
* זמינות נקראת תמיד משדה `availability` של המוצר, וזמני המסירה ועלות המשלוח
  ...
  מתוצאת כלי באותו תור. מבצעים, החזרות, אחריות ותמחור הזמנה אישית אינם ידועים
