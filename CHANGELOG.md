...
* עמוד `/returns-service` עם החלפה בתוך 30 יום, החזר מלא בתוך 14 יום, שילוח
  ...
  בלאי לאחר הצעת מחיר, וניקוי מקצועי ובדיקת שיבוץ ללא עלות. הנתונים מרוכזים
  ב־`src/lib/servicePolicies.ts`.
* מספר קטלוגי ייחודי לכל 16 המוצרים, בקידומות קבועות לפי קטגוריה. המספר מוצג
  ליד שם המוצר ונשמר בעגלה, בצ׳קאאוט ובאישור; כלי הקטלוג של העוזר יכול להחזיר
  אותו מתוך `products.ts` באותו תור.
* קישור שקט מעמוד מוצר לצפייה בתכשיט באטלייה, ושדה הערות חופשי ואופציונלי
  להזמנה או למשלוח ליד אזור ההקדשה. ההערה נשמרת רק ב־snapshot המקומי של
  הזמנת ההדגמה ומוצגת באישור כשהיא אינה ריקה.
* בחירת 14 או 18 קראט בשבעה דגמים מתאימים, עם שבעה מחירי 18 קראט מפורשים.
  בחירת 18 קראט מעדכנת יחד מחיר, זמינות להזמנה מיוחדת וזמן ייצור, והקראט נשמר
  ומוצג בעגלה, בצ׳קאאוט ובאישור ההזמנה. תשעת הדגמים האחרים נשארים ב־14 קראט
  ומציגים הסבר קצר על קשיחות והתאמה לענידה רצופה.
* תיבת בחירה של 1 עד 12 תשלומים בכרטיס אשראי, ללא תוספת למחיר. החלוקה שומרת
  על הסכום המדויק גם כשאין חלוקה שלמה, ונרשמת באישור ההזמנה; Bit ו־Apple Pay
  נשארים בתשלום אחד.

* נתוני חומרים מפורשים לכל 16 המוצרים: סוג ומקור האבנים, משקל קראט כולל,
  צבע וניקיון לסוליטר, ומשקל זהב נקי משוער בגרמים. העוזר יכול להציג את פרטי
  האבנים ומשקל הזהב רק מתוך תוצאת כלי באותו תור.
...
* עמוד `/visit` לביקור בסטודיו בשבזי 45: מפה מוטמעת, ניווט ב־Waze, שעות
  פתיחה, המלצה לתאם מראש וקישור WhatsApp דרך מקור הקשר המשותף.
...
* פעולות מחוברות לאפליקציה האמיתית: מעבר לעמוד מוצר דרך react-router, הוספה
  ...
  לוואטסאפ דרך `whatsappUrl()` המשותף.
...
* האשף הדטרמיניסטי של עוזר הבחירה הוסר. הוא לא ידע לפרש טקסט חופשי ולכן לא
  שימש עוד כרשת ביטחון אמינה לאחר הוספת Gemini. יחד איתו הוסרו שאלות הבחירה,
  מצב הסינון, פעולת ההתחלה מחדש ופונקציות ההרחבה שהיו ייחודיות לו.

### Changed

* כשל סופי במסלול Gemini אינו פותח עוד שיחה חלופית. הוא מציג הודעה עברית
  קצרה ופעולת `catalog` טיפוסית שמנווטת דרך `ROUTES.catalog`. סולם ההתאוששות
  נשמר: כשל תחבורה ראשון משאיר את Gemini פעיל ורק השני ברצף הופך לסופי.
  כפתור החזרה ממשיך לשחזר snapshot של שיחת Gemini ומחזיר את הפוקוס לשדה
  הטקסט.
* לוגי הפונקציה כוללים כעת זמני כל קריאת Gemini, מונה השימוש, כל קריאת כלי
  והזמן הכולל של הפנייה, לצד תוצאת הכשל ומספר ה־retries. לא נרשמים תוכן
  משתמשת, מפתח API או מזהה סשן.

* חיפוש התוכן של העוזר תומך כעת בשאלות שמחברות כמה נושאים: כל נושא מקבל
  קבוצת מונחים משלו, התוצאות מחולקות בסבב בין הנושאים והמונחים, וקטעים מאותו
  מקור מתאחדים במקום להסתיר זה את זה. נוספה הרחבת נטיות ומילים נרדפות בשכבת
  האחזור, וערכים דינמיים כמו שעות, מסירה, שירותים, עיצוב אישי וסיפור דנה
  נקראים ממקורות האמת המשותפים. אין מסווג כוונות או תשובות מוכנות.
* תשובות Gemini מוגדרות לעברית ישראלית בלבד ונבדקות לפני החזרה לאותיות
  לטיניות, קיריליות, ערביות או יווניות. הטמפרטורה הופחתה כדי לצמצם ערבוב
  שפות ושגיאות ניסוח, בלי לשנות את מנגנון ההארקה של נתוני המוצרים.

* הודעת טקסט שנשלחת לעוזר מוצגת מיד בתמליל ללא חיווי ״חשיבה״ כללי. Gemini
  מקבל את ההודעה ישירות עם כלי האתר במצב `AUTO`; רק קריאה ממשית לכלי נתונים
  מזרימה `checking-site` ומציגה ״בודקת באתר...״. ברכה ושיחה רגילה אינן מציגות
  חיפוש. פונקציית הסוכן נבנתה מחדש מבסיס `39d7a91`: אין קריאת ביניים, marker,
  timeout או retry יזומים. מלוא היסטוריית השיחה משני הצדדים נשלחת ל־Gemini
  לצורך המשכיות בלבד, ועובדות עסקיות עדיין מחייבות כלי חדש באותו תור. מנגנון
  ההארקה לא השתנה.
* עוזר Gemini אינו מסווג עוד הודעות לפני המודל: ניתוב קנייה/מדיניות/נושא לא
  ...
  ב־`AUTO`; Gemini מנהל שיחה ובוחר כלים. כלי העסק מחזירים לו מידע גולמי
  ממקורות האתר והוא מנסח את התשובה בעצמו, בלי תבניות תשובה בקוד. נוסף מקור
  משותף לעיצוב אישי, והמלצות יכולות להיבחר לפי הרלוונטיות של הרשומות שחזרו
  תוך אימות slugs, סדר קטלוג יציב ומגבלה של שלושה כרטיסים. מנגנון ההארקה
  וה־fallback לאשף נשארו ללא שינוי.
* פאנל עוזר הבחירה מגיב כעת ל־VisualViewport של iOS Safari בזמן שהמקלדת
  ...
  הגובה הרגיל. בדיקת הרגרסיה מדמה layout viewport מלא לצד VisualViewport
  מוקטן ומוזז, ולא רק שינוי גודל רגיל של Chromium.
* פאנל עוזר הבחירה הוגבל במובייל לרוחב ה־viewport: בועות עברית ארוכות נשברות
  בתוך הפאנל, ה־header ושורת הקלט נשארים בגבולותיו, וכפתורי הסגירה והשליחה
  אינם יכולים להתכווץ או לצאת מהמסך. גודל שדה הקלט במובייל הועלה ל־16px כדי
  למנוע זום אוטומטי של iOS Safari. בדיקת רגרסיה מודדת את הרוחבים בפועל עם
  הודעה ותשובה ארוכות; בדיקת המדיניות מאמתת את הקישור בכל 16 עמודי המוצר
  ובפוטר.
* הקישור מתוך ״החלפה תוך 30 יום״ בפס האמון מסומן כקישור גם במגע, ולא רק
  במצב hover שאינו קיים ב־iPhone. הוא ממשיך להפנות לאותו עמוד תנאים בכל
  16 המוצרים; עמוד התנאים נשאר מקושר גם מהפוטר.
* הבטחת ההחלפה בדף הבית ובפס האמון מקושרת לעמוד התנאים. מוצר שנוצר בהזמנה
  מציג לפני הקנייה את מגבלת ההחלפה וההחזר לאחר תחילת הייצור; מוצר שאזל אינו
  מציג מסירה, איסוף או משלוח. שירות התאמת מידה מופיע בעמודי טבעות ובמדריכי
  המידות, ושירות הניקוי ובדיקת השיבוץ מופיע בכל עמוד מוצר.
* העגלה ומגירת העגלה מציינות שאיסוף חינם מהסטודיו נבחר בצ׳קאאוט. הפוטר מציג
  את כתובת האטלייה ושעות הפתיחה ומקשר לעמוד ההחלפות והשירות.
* `search_products` יכול להחזיר מדיניות החלפה, החזר, התאמת מידה, אחריות,
  תיקונים וניקוי מתוך המקור המשותף באותו תור. ניסוח העובדות נבנה בקוד לאחר
  סריקת הפרוזה הקיימת; מנגנון ההארקה לא שונה. רק מבצעים ותמחור אישי לא מפורש
  נשארים נושאים בלתי ידועים.
* מספר ה־WhatsApp הוחלף למספר הפעיל `050-9054826`; כל הקישורים, הכפתור הצף,
  עמוד הביקור, הצהרת הנגישות והפניות העוזר משתמשים ב־`972509054826` דרך
  `whatsappUrl()`. האימייל והאינסטגרם נשארו פרטי הדגמה ומסומנים כך בכל מקום
  שבו הם מוצגים.
* אישור ההזמנה מצהיר כעת שהפעולה היא הדגמה בלבד ושלא בוצעו חיוב, אימייל,
  טיפול או משלוח. טופסי הניוזלטר והעיצוב האישי מציגים מראש ובאישור שהקלט
  נבדק מקומית ואינו נשלח או נשמר. כפתור טופס החזרה למלאי מציין כעת בדיקת
  כתובת במקום שמירה, בהתאם להתנהגות המקומית שלו.
* כלי המוצר של העוזר מחזיר מחיר 18 קראט, זמן ייצור ונימוק ל־14 קראט בלבד.
  המחיר מוצג בכרטיס קטלוג שמרונדר מהנתונים, והעובדות האחרות בתבנית קוד קבועה;
  מנגנון ההארקה של פרוזת המודל לא שונה.
* ההבטחה להתאמת גודל אבן הוסרה מעמוד הסיפור. התאמות מתכת ומידה נשארו, ולא
  נמצא באתר מקום נוסף שמבטיח התאמת גודל או סוג אבן.
* טווח המחירים המתועד תוקן ל־₪890–₪10,200, כולל גרסת 18 קראט של הסוליטר,
  וטקסטי המותג מתארים 18 קראט כהזמנה מיוחדת בדגמים נבחרים.
* הוראת השחזור הישנה ב־`docs/IMAGE_ASSET_REGISTER.md` עודכנה למבנה האבנים
  המפורש הנוכחי, בלי לשנות את תיעוד התמונות עצמו.

* תיאור האבנים בעמוד מוצר מגיע תמיד מרשומת המוצר; הוסרה ברירת המחדל שטענה
  בטעות ליהלומים טבעיים. תעודת יהלום מוצגת בפס האמון רק ליהלום יחיד מעל
  0.3 קראט, בהתאם להצהרה בדף הבית.
* המחירים עודכנו לארבעה מוצרים: `stacking-rings` ל־₪3,900,
  `beaded-bracelet` ל־₪2,400, `slim-bangle` ל־₪5,900 ו־
  `fine-diamond-band` ל־₪3,400.
* מחיר `single-diamond-necklace` עודכן מ־₪2,450 ל־₪3,400, מעל מחירי חנות
  רגילה לשרשרת עם יהלום טבעי 0.25 קראט ומתחת למוצרי פרימיום מתועדים.
* ההצהרה הכללית על זהב 14 ו־18 קראט הוחלפה בדף הבית, במטא, ב־JSON-LD
  ובמדריך הטיפוגרפיה בניסוח מדויק: הקולקציה בזהב 14 קראט, ו־18 קראט זמין
  לפי בקשה רק בדגמים נבחרים שנוצרים בהזמנה. כלי הקטלוג של העוזר מחזיר כעת
  גם את `availableIn18K`, והתשובה מוצגת בתבנית קוד קבועה מתוך נתוני המוצר.
* מדיניות המשלוח שונתה למשלוח עד הבית ללא עלות בכל הזמנה. פס האמון, עמוד
  המוצר, העגלה, הצ׳קאאוט, אישור ההזמנה והעוזר קוראים את המחיר האפסי ממקור
  המשלוח המשותף; רכיב ההתקדמות למשלוח חינם הוסר.
...
* עלות המשלוח רוכזה לצד זמני המסירה ב־`src/lib/fulfillment.ts`: משלוח לבית
  ואיסוף מהסטודיו הם ללא עלות בכל הזמנה. המחיר מוצג בעמוד המוצר, במגירת
  העגלה, בעמוד העגלה ובסיכום הצ׳קאאוט.
* עוזר הבחירה עונה על זמינות, זמני מסירה ועלות משלוח מהקטלוג ומהקבועים
  המשותפים בלבד. מבצעים, החזרות ואחריות נשארים בלתי ידועים ומועברים
  ל־WhatsApp; מנגנון ההארקה הקיים לא שונה.
...
* תוצאות הקטלוג נעשו דטרמיניסטיות לאחר בחירת המסננים: מיון קבוע ותת־קבוצה של
  שלושת הראשונים נקבעים בקוד, ו־Gemini אינו יכול לשנות סדר או לבחור מועדפים.
  זמינות וזמני מסירה נשארים בתבניות קוד קבועות. שאלת הבהרה מוגבלת מבנית
  לממד אחד, וניסוחים ביורוקרטיים והפניית WhatsApp הוחלפו בעברית טבעית יותר.
...
* לעוזר יש כעת backend יחיד ב־Netlify Functions. `GEMINI_API_KEY` נקרא רק
  ...
  במבנה הקודם.
* העוזר מצהיר על זמינות, זמני מסירה ועלות משלוח רק מתוך `products.ts`
  והקבועים המשותפים. מבצעים, החזרות ואחריות עדיין אינם ידועים; שאלה עליהם
  מקבלת תשובה כנה והפניה לוואטסאפ.
