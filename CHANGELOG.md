...
* הודעת טקסט שנשלחת לעוזר מוצגת מיד בתמליל ללא חיווי ״חשיבה״ כללי. ה־Function
  שולח תחילה קריאת Gemini קלה ללא סכמות כלי האתר. רק אם Gemini עצמו מחזיר
  marker פנימי שנדרש מידע מנוגה, מוזרם `checking-site`, נטענים הכלים המלאים
  ומוצג ״בודקת באתר...״. ברכה ושיחה רגילה אינן מציגות חיפוש. נוספו timeout של
  8 שניות לקריאה הקלה ו־12 שניות לכל קריאת אתר כדי שכשל ספק לא יישאר תלוי
  ללא סוף. החיווי אינו תבנית תשובה ואינו משנה את מנגנון ההארקה.
* הסימון הפנימי לבדיקת האתר מזוהה גם עם תווים נסתרים או עטיפת Markdown ונחסם
  גם בצד השרת וגם בצד הלקוח. נוסף ניסיון חוזר יחיד כאשר נתוני האתר כבר התקבלו
  אבל קריאת הניסוח של Gemini נכשלה זמנית.
