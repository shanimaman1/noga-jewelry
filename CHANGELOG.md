...
* שכבות z: קבוצת הפעולות הצפות (`FloatingActions`) ו־scrim 30, פאנל 41,
  ...
  בשום גובה מסך.
* **בדיקה אמפירית של Gemini Flash לקראת שלב 2 (עוזר מבוסס LLM) — שלב 2 עדיין
  לא מאושר ולא נבנה.** סקריפט חד-פעמי וזמני, שלא הוכנס לגיט (הורץ תחת
  ...
  בקשות לדקה / 20 בקשות ליום** — נמוך מדי לעוזר חי. גרסאות `flash-lite` לא
  נבדקו אמפירית. יש לבדוק את המכסה מחדש לפני כל המשך עבודה על שלב 2.
